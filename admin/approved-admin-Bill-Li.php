...
<?php
      ...
      $sql_requisition_depart_approve_log = $db->query("SELECT c.id, c.reference, c.requesterID, c.requesterName, c.designation, c.department, c.section, c.costDivision, c.PurchaseFor, c.ruqest_date, c.email FROM canteen_header c LEFT JOIN user_role r ON c.PurchaseFor = r.category and c.costDivision=r.companyID LEFT JOIN canteen_line a ON c.reference =a.reference LEFT JOIN approve_verify ap ON c.reference = ap.reference and r.employeeID=ap.userID where r.user_name='$username' and c.status='1' and r.user_type='Approval User' and ap.userID='$employeeID' and c.approvedStatus BETWEEN 4 AND 6 GROUP BY c.id");
