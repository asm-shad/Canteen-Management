...
<div class="panel-heading">
    <h3 class="panel-title">Requisition Bill Approved List</h3>
...
<?php
    ...
    $sql_requisition_depart_approve_log = $db->query("SELECT c.id,  c.reference, c.requesterID, c.requesterName, c.designation, c.department, c.section, c.costDivision, c.PurchaseFor, c.ruqest_date, c.email, c.approvedStatus, c.rejectFrom
      FROM canteen_header c
      LEFT JOIN user_role r
      ON c.PurchaseFor = r.category and c.costDivision=r.companyID LEFT JOIN canteen_line a ON c.reference =a.reference where r.user_name='$username' and c.status='1' and c.approvedStatus BETWEEN 6 and 7 GROUP BY c.id");
            ...
            ?>
                ...
                <td><center>
                         <?php if($requisition_data['approvedStatus'] == '6') {
                            echo "Purcahse has been done";
                        } 
                    ...
                    <td><center>
                    <a href="auditdview.php?view_id=<?php echo md5($requisition_data['id']); ?>" class="btn btn-primary btn-sm">View</a>
                    </center></td>
